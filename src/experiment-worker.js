...
onmessage = (e) => {
  ...
  const aggregates = {
    AA: {
      count: 0,
      step1: { numberCompleted: 0, percentageCompleted: 0 },
      step2: { numberCompleted: 0, percentageCompleted: 0 },
    },
    AB: {
      count: 0,
      step1: { numberCompleted: 0, percentageCompleted: 0 },
  ...
  for (var i = 0; i < count; i++) {
    ...
    if (run.completedStep1) {
      ...
      aggregate.step1.percentageCompleted = Math.round(
        (100 * aggregate.step1.numberCompleted) / aggregate.count
      );
    }

    if (run.completedStep2) {
      aggregate.step2.numberCompleted += 1;
      aggregate.step2.percentageCompleted = Math.round(
        (100 * aggregate.step2.numberCompleted) / aggregate.count
      );
    }
